...
class _SoftDTW(Function):
    ...
    def forward(ctx, D, gamma, bandwidth):
        dev = D.device
        dtype = D.dtype
        gamma = torch.FloatTensor([gamma]).to(dev).type(dtype)  # Updated for CPU compatibility
        bandwidth = torch.FloatTensor([bandwidth]).to(dev).type(dtype)
        D_ = D.detach().cpu().numpy()
        g_ = gamma.item()
        b_ = bandwidth.item()
        R = torch.FloatTensor(compute_softdtw(D_, g_, b_)).to(dev).type(dtype)
    ...
    def backward(ctx, grad_output):
        ...
        g_ = gamma.item()
        b_ = bandwidth.item()
        E = torch.FloatTensor(compute_softdtw_backward(D_, R_, g_, b_)).to(dev).type(dtype)
        return grad_output.view(-1, 1, 1).expand_as(E) * E, None, None
